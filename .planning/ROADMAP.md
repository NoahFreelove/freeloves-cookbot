...
- 📋 **v1.5 External Agent Interface (MCP + REST API)** — planning (next via `/gsd-new-milestone`)
...
### 📋 v1.5 External Agent Interface (MCP + REST API) — PLANNING

Phases to be defined by `/gsd-new-milestone` (continues numbering from Phase 16). Target: let external AI agents manage the pantry and create recipes via a shared agent-operations facade over `PantryService`/`RecipeService`, exposed as both a REST minimal-API and an in-process MCP server, with per-agent API-token auth (token→user; no Identity/OAuth) and structured-submit recipe creation (canonical `RecipeDocument` → schema-validate → `RecipeService.CreateAsync`, no inbound AI).
...
*v1.4 shipped 2026-06-25 (tag `v1.4`) — Phases 12–16, 18 plans, 21/22 requirements (GALLERY-04 retired; UATAUTO-02 partial). Full detail archived to [`milestones/v1.4-ROADMAP.md`](milestones/v1.4-ROADMAP.md). Next: v1.5 External Agent Interface via `/gsd-new-milestone` (continues phase numbering from 16).*
