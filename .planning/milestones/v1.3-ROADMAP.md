...
- 📋 **v1.3 Production-Ready & Format Maturity** — Phases 8–11 (63 reqs + Phase 11 UAT cleanup)
...
- [x] **Phase 11: v1.3 UAT Cleanup & Automated UAT Harness** — Close four Phase-10-UAT-surfaced items (RecipeView inline Edit button, RecipeView responsive layout ≤720px, sidebar Profile-row clip + body-bg, unit-system display conversion) and stand up a reusable Playwright/Node browser-UAT harness (promoted from backlog 999.2–999.5 on 2026-06-05) (completed 2026-06-05)
...
### Phase 11: v1.3 UAT Cleanup & Automated UAT Harness

**Goal**: The four defects/gaps surfaced during Phase 10 UAT are closed, and the project gains a reusable browser-UAT harness so future human-UAT flows run hands-free. Specifically: the Edit button is present in the RecipeView inline-fallback action row at narrow viewports; RecipeView's whole layout responsively collapses ≤720px (not just the TopBar toggle); the sidebar Profile row is fully visible and the cream body background reaches the sidebar bottom; recipe amounts and per-step temperatures render in the user's selected unit system on RecipeView / CookingMode / AiChat canvas with a per-recipe opt-back-to-original toggle; and a Playwright/Node harness drives chromium through the still-open Phase 10 UAT reruns (Tests 5 and 7) automatically.
**Depends on**: Phase 10 (all four items are Phase 10 surfaces; harness re-runs Phase 10 UAT). Source detail preserved in `phases/11-v1.3-uat-cleanup/11-BACKLOG-SOURCE.md`.
**Requirements**: CLEANUP-01, CLEANUP-02, CLEANUP-03, CLEANUP-04, UATAUTO-01
**Success Criteria** (what must be TRUE):

  1. At ≤720px viewport on `/recipes/{id}`, the inline-fallback action row shows Edit alongside Share / Schedule / Cook this — root cause of the missing Edit button found and fixed, not worked around (CLEANUP-01, was 999.5)
  2. At ≤720px on `/recipes/{id}` the hero grid stacks to one column, ingredient/method columns reflow to full width, and the step-number grid no longer wraps per-word — RecipeView is usable, not "squished" (CLEANUP-02, was 999.4)
  3. At default desktop zoom the sidebar Profile row is fully visible (no left-clip) and the `--cream` body background extends to the full sidebar height (CLEANUP-03, was 999.3)
  4. With `UserProfile.UnitSystem` set, `RecipeIngredient.Amount`/`Unit` and per-step `StepTemperature` display converted on RecipeView, CookingMode, and the AiChat canvas via a real conversion layer (g↔oz, ml↔fl oz, °F↔°C, gas mark), with a per-recipe toggle back to the AI-emitted original units; canonical `RecipeDocument` is never mutated (CLEANUP-04, was 999.2)
  5. A Playwright/Node harness, isolated from the shipped app and the .NET solution build, launches the app and drives chromium through UAT Test 5 (cookbook reparenting) and UAT Test 7 (responsive collapse) end-to-end with pass/fail assertions; UAT Test 4's validation-fail path is either fault-injected or explicitly recorded as a manual/deferred check — not faked (UATAUTO-01)

**Plans**: 5 plans in 2 waves

- [x] 11-01-PLAN.md — Wave 1: RecipeUnitDisplayService — °C/°F/gas temperature table + weight/volume conversion engine + tests (CLEANUP-04 engine)
- [x] 11-02-PLAN.md — Wave 1: RecipeView inline-fallback Edit flex-wrap fix + responsive single-column collapse at ≤720px (CLEANUP-01, CLEANUP-02)
- [x] 11-05-PLAN.md — Wave 1: Playwright/Node UAT harness — Tests 5 & 7 automated, Test 4 honesty disposition (UATAUTO-01)
- [x] 11-03-PLAN.md — Wave 2: Sidebar Profile-row clip + cream body-bg height-inheritance fix (CLEANUP-03)
- [x] 11-04-PLAN.md — Wave 2: Wire unit conversion into RecipeView / CookingMode / AiChat + per-recipe localStorage toggle (CLEANUP-04 wiring)
